...
- **Pure modules import no Zeus21.** `priors.py`, `select.py`, `provenance.py` must import only numpy/scipy/stdlib so their tests run under the main env.
...
### Task 2: `select.py` — reionization cut and figure subsample

**Files:**
- Create: `models_21cm/select.py`
- Test: `models_21cm/test_select.py`
...
Run: `uv run pytest models_21cm/test_select.py -v`
Expected: FAIL — `AttributeError: module 'select' has no attribute 'reionized'` (Python's stdlib `select` shadows it until our file is importable from the test directory; pytest's `prepend` import mode puts `models_21cm/` at `sys.path[0]`, so our module wins once it exists)

- [ ] **Step 3: Write minimal implementation**

```python
# models_21cm/select.py
...
```bash
uv run ruff check models_21cm/ && uv run ruff format models_21cm/
git add models_21cm/select.py models_21cm/test_select.py
...
def test_neutral_fraction_is_exposed_and_complete(fiducial):
    """select.reionized() depends on this attribute existing."""
...
Only this module imports zeus21; priors/select/provenance stay pure so
...
**Interfaces:**
- Consumes: the production npz; `provenance.rebuild_params`, `provenance.parse_header`; `select.reionized`; `foreground_svd.npz` and `horizon_shift.npz` from the paper repo.
...
def load_t21(freqs):
    ...
    keep = sel.reionized(m["xHI"], m["z_xHI"])
...
The pure modules (`priors.py`, `select.py`, `provenance.py`) import no
...
- `priors.py`, `select.py`, `provenance.py` are **pure** — numpy/scipy/
