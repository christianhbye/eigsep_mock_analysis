...
def build_notebook():
    md = (
        ...
        "cut requiring xHI below threshold both at the dark-pixel "
        "reference redshift (z = 5.9) and at the top of the observed band "
        "(z = 4.6816, 250 MHz). The z = 5.9 threshold of 0.1 is roughly "
        "half the current model-independent limit (xHI <= 0.191 + 0.056 "
        "at z = 5.831; Davies+2025, arXiv:2510.25829, superseding "
        "McGreer+2015), so the cut is conservative -- and it is not "
        "load-bearing: sweeping the threshold from 0.06 to 0.25, or "
        "dropping the z ~ 6 anchor entirely, leaves the operating point "
        "at N = 9 and holds the above-floor fraction within 0.8 points "
        "across the range the current data allow (3.7 points with no "
        "anchor at all). See "
        "`horizon_position/reionization_sensitivity.py`. "
        "The npz carries its own regeneration recipe "
